...
def parse_args() -> argparse.Namespace:
    """
    Parse command line arguments for CIFAR-10 classification.
    ...
    parser.add_argument('--model-path', type=Path, default=Path('cifar10_model_final.safetensors'), help='Path to save the final model')
...
class NeuralNetwork(nn.Module):
    """
    Simple feedforward neural network for CIFAR-10 classification.
    ...
    def __init__(self, dropout_rate: float=0., num_hidden_layers: int=2, hidden_layer_size: int=64) -> None:
        ...
        for i in range(num_hidden_layers):
            ...
            input_size = out_size
        layers.append(nn.Linear(input_size, 10))  # 10 classes for CIFAR-10
...
def compute_mean_std(dataset: torch.utils.data.Dataset) -> tuple[list[float], list[float]]:
    """
    Compute the per-channel mean and std for a dataset.

    Parameters
    ----------
    dataset : torch.utils.data.Dataset
        Dataset of images (expects images as PIL.Image or torch.Tensor).
...
def train_model(args: argparse.Namespace) -> None:
    """
    Train the neural network model on the CIFAR-10 dataset.
    ...
    try:
        ...
        task = Task.init(
            ...
            task_name="CIFAR-10 Dense Layers"
        )
    ...
    data_dir = Path("data")
    # First, load with ToTensor only to compute mean/std
    ...
    try:
        ...
        # Now use these for normalization
        transform = Compose([
            ToTensor(),
            Normalize(mean, std)
        ])
        training_data = datasets.CIFAR10(root=data_dir, train=True, download=False, transform=transform)
    ...
    for epoch in range(args.epochs):
        ...
        if task:
            cm = confusion_matrix(all_labels, all_preds, labels=list(range(10))) # 10 classes for CIFAR-10
...
if __name__ == "__main__":
    """
    Main entry point for training and testing the CIFAR-10 classifier.
    """
    try:
